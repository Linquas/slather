...
module Slather
  class Project < Xcodeproj::Project
    ...
    attr_accessor :build_directory, :ignore_list, :ci_service, :coverage_service, :coverage_access_token, :source_directory,
      ...
      :decimals, :path_equivalence, :llvm_version, :configuration
    ...
    def pathnames_per_binary(binary_path)
      ...
      coverage_json["data"].reduce([]) do |result, chunk|
        result.concat(chunk["files"].map do |file|
          ...
          filename = replace_pathname_with_path_equivalence_if_needed(filename)
    ...
    def replace_pathname_with_path_equivalence_if_needed(pathname)
      if self.path_equivalence && pathname
        name = pathname.to_s
        # slpit the path_equivalence into two parts with ',' as the delimiter.
        pathToBeReplaced = self.path_equivalence.split(",")[0]
        localPath = self.path_equivalence.split(",")[1]
        # if filename has the prefix of pathToBeReplaced, replace it with localPath
        if name.start_with?(pathToBeReplaced) 
          name = localPath + name[pathToBeReplaced.length..-1]
        end
        Pathname.new(name)
      else
        pathname
    ...
    def create_coverage_files(binary_path, path_objects)
      ...
      files.map do |source|
        ...
        # If path_equivalence is set, replace the path in the coverage file with the local path.
        coverage_file.source_file_pathname = replace_pathname_with_path_equivalence_if_needed(coverage_file.source_file_pathname)
    ...
    def configure_path_equivalence
      self.path_equivalence ||= self.class.yml["path_equivalence"] if self.class.yml["path_equivalence"]
    end
